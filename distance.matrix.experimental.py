...
def estimate_drive_minutes(origin_coords, dest_coords):
    if not origin_coords or not dest_coords:
        return 999
    lat1 = math.radians(float(origin_coords["lat"]))
    lat2 = math.radians(float(dest_coords["lat"]))
    dlat = math.radians(float(dest_coords["lat"]) - float(origin_coords["lat"]))
    dlng = math.radians(float(dest_coords["lng"]) - float(origin_coords["lng"]))
        ...
        if origin_coords:
            for start in range(0, len(full_addresses), destination_chunk_size):
                ...
                for offset, dest in enumerate(dest_addresses):
                    ...
                    if not dest_coords:
                        continue
                    row_minutes[j] = estimate_drive_minutes(origin_coords, dest_coords)
                    dest_estimates[j] = row_minutes[j]
                    ...
                    if dm_res["status"] == "OK":
                        ...
                        for dest_index, element in zip(dest_positions, elements):
                            if element["status"] == "OK":
                                ...
                                row_minutes[dest_index] = dest_estimates.get(dest_index, row_minutes[dest_index])
                except Exception:
                    pass
