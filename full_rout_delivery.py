...
def run_multi_cluster_tsp_test():
    ...
    # 4. Start journey
    print("\n" + "="*60)
    print("🚀 STARTING MULTI-VEHICLE TSP JOURNEY (TEST MODE)")
    print("="*60)
        ...
        while driver_time < MAX_SHIFT_MINUTES and len(completed_clusters) < total_clusters:
            ...
            if closest_info is None:
                if active_city is not None:
                    print(f"\n🏙️  City '{active_city}' is complete. Searching globally for next city...")
                    active_city = None
                    continue
                else:
                    break
            ...
            if active_city is not None and next_city != active_city:
                print(f"\n🏙️  City '{active_city}' is complete! Moving to new city: '{next_city}'")
            ...
            print(f"   City: {active_city} | Cluster: {cluster_group} | Time so far: {driver_time:.2f}/420 mins")
            ...
            if len(matrix_df.columns) == 1:
                ...
                step_travel_time = np.random.randint(1, 8)
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        for d_id, data in all_drivers_results.items():
            summary_data.append({
                ...
                'Mode': "TEST (Random times)" if test_mode else "Production (API)"
            })
