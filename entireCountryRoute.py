...
def export_journey_to_excel(all_drivers_results, df_addresses, test_mode=False):
    ...
    base_filename = "multi_vehicle_journey_TEST" if test_mode else "multi_vehicle_journey"
    excel_path = Path.home() / "Desktop" / f"{base_filename}.xlsx"
    ...
    # Try saving, fallback if file is open in Excel
    saved = False
    while not saved:
        try:
            with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            ...
            print(f"\n⚠️ Permission denied! The file '{excel_path.name}' is likely open in Excel.")
            excel_path = Path.home() / "Desktop" / f"{base_filename}_{int(time.time())}.xlsx"
            print(f"🔄 Automatically saving to a new file instead: {excel_path.name}")
